...
contract Wallet is BaseAccount, Initializable, UUPSUpgradeable, TokenCallbackHandler {
    ...
    function _call(address target, uint256 value, bytes memory data) internal {
        ...
        if (!success) {
            assembly {
                // The assembly code here skips the first 32 bytes of the result, which contaons the lenght of data.
                // It then loads the actual error message using mload and calls revert with this errir message
    ...
    {
        require(dests.length == funcs.length, "wrong dests lenghts");
